        ...
        <div className="px-6 pt-4 pb-10 space-y-3 bg-[#0F172A] border-b border-white/10 shadow-2xl">
          {navLinks.map((link) => (
            <NavLink
              ...
              to={link.to}
              onClick={closeMenu}
              className={({ isActive }) =>`block w-full text-right px-4 py-4 rounded-xl text-base font-bold transition-colors ${
                isActive ? 'bg-[#B89544]/10 text-[#B89544]' : 'text-slate-300 hover:bg-white/5'
              }`}
            >
              {link.text}
            </NavLink>
