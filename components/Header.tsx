...
const Logo: React.FC<{ light?: boolean }> = ({ light = true }) => (
    <Link to="/" className="flex items-center space-x-3 rtl:space-x-reverse focus:outline-none group">
        <div className="bg-gradient-to-br from-[#B89544] to-[#D4AF37] p-1.5 md:p-2 rounded-xl shadow-lg shadow-[#B89544]/20 transition-transform duration-300 group-hover:scale-105">
          <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 md:h-6 md:w-6 text-[#0F172A]" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}>
...
const Header: React.FC<HeaderProps> = ({ onOpenModal }) => {
  ...
  return (
    <header className={`fixed top-0 left-0 right-0 z-[100] transition-all duration-500 h-16 md:h-24 ${
      scrolled 
            ...
            <div className="space-y-1">
              {navLinks.map((link, idx) => (
                <NavLink
                  ...
                  to={link.to}
                  onClick={closeMenu}
                  className={({ isActive }) => `flex items-center px-6 py-4.5 rounded-2xl text-[17px] font-bold transition-all duration-300 ${
                    isActive ? 'bg-[#B89544] text-[#0F172A] shadow-lg shadow-[#B89544]/20' : 'text-slate-300 hover:bg-white/5 hover:text-white'
                  }`}
                >
                  {link.text}
                </NavLink>
